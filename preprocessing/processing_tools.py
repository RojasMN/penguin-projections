...
def filter_data(data):
    ...
    peng_type = data['name_id'].unique()[0]
    print(f'FILTERING DATA: {peng_type.upper()} PENGUINS.')
...
def select_good_chunks(data):
    ...
    print(f'A TOTAL OF {len(dataframes)} GOOD CHUNKS WERE FOUND.')
...
def full_pipeline(data, species):
    ...
    """
    Runs the full preprocessing pipeline for time series data of a specified penguin species.

    This function filters, segments, interpolates, and saves time series data related to 
    penguin counts. It accepts a dataframe containing raw data from multiple penguin species, 
    extracts the subset for the target species, and applies a series of preprocessing steps. 
    The output consists of cleaned and interpolated time series saved as separate .csv files.
    ...
    if data_sp.empty:
        print(f"NO DATA FOUND FOR: '{species.upper()}'.")
        print('SKIPPING PREPROCESSING.')
        return
    
    # Processing and saving
    data_filtered = filter_data(data_sp)
    subsets = select_good_chunks(data_filtered)
    subsets_interp = interpolate_all_subsets(subsets)
    save_subsets_csv(subsets_interp)
    
    
def process_count_data():
    
    """
    Executes the full preprocessing pipeline for all predefined penguin species.

    This function reads raw penguin count data from a .csv file and processes it species by species.
    For each target species, it calls the 'full_pipeline' function to filter, clean, interpolate, 
    and save the corresponding time series data as separate .csv files.
    
    """
    
    query_path = r'C:\Users\marce\Escritorio\Proyectos\projections_pys\data\query_all_counts.csv'
    sp_ids = ['adelie', 'chinstrap', 'macaroni', 'gentoo', 'emperor', 'king', 'rockhopper']
